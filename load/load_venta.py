...
def cargar_fact_venta():
    ...
    try:
        ...
        venta_tra = pd.read_sql("SELECT * FROM tra_Venta", ses_db_stg)
        ...
        table_name_sor = 'Fact_Venta'

        # Fusionar (merge) los datos transformados en la tabla Fact_Venta de sor
        venta_tra.to_sql(table_name_sor, ses_db_sor, if_exists='replace', index=False, method='multi')
